...
class MaxDamagePlayer(Player):
    ...
    # new function for team choosing
    def teampreview(self, battle):
        ...
        # sort our mons by performance
        ordered_mons = sorted(mon_performance, key = lambda k: -mon_performance[k])

        # we start with the one we consider best overall
        # use i+1 as python indexes start from 0, but showdown starts from 1
        return "/team " + ''.join([str(i + 1) for i in ordered_mons])



# helper for which lead to choose based on type advantages
def teampreview_performance(mon_a, mon_b):
    # we first evaluate the performance on mon_a against mon_b as its type advantage
